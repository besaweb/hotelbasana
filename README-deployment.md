...
- Kontaktní formulář (`send-request.php`) — server-side validace, honeypot proti spamu, odesílá e-mail na `info@hotelbasana.al` přes PHP `mail()`. Frontend posílá AJAX (fetch) a zobrazuje přeložený stavový text.
...
1. **Doména** — v `index.html` je canonical/OG/JSON-LD URL nastavena na `https://www.hotelbasana.al/` jako odhad podle e-mailové domény. Uprav na skutečnou doménu, jakmile ji budeš mít.
